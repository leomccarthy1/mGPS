---
title: "Mapping with microbes with machine learning and XBoost"
output: html_notebook
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
knitr::opts_knit$set( root.dir = "/Users/leomccarthy/Documents/Microbiome mapping/Markdown" )
knitr::opts_chunk$set(cache =TRUE)
```

This is a comprehensive outline of the main modeling methods used in our peper titled "" in order to assist in reproducibility of results. 
...
There are a group of samples from small towns accross uk, we will remove these as there are very few from each town and they will clutter up results A LOT while ading little improvement to a model on a global scale 
...
Not all of the samples have exact origin co-ordinates recorded, for the purposes of global predictions we can substitute the central co-ordinates of the city of origin for the missing exact co-ordiantes, found in the city meta-data. On the global scale this shouldn't be too much of an issue. 
...
To select an optimum set of variables for each of the target variables of interest (city,latitude and longitude) we will use recursive feature elimination with random forests. The caret function "rfe" uses re-sampling and external validation to protect against selecting features that lead us to overfit the training data. More information is avalable at  https://topepo.github.io/caret/recursive-feature-elimination.html 
...
Here we can see the effect of selecting smaller subsets of features on preict city of origin. 

```{r}
plot(featureElimination, type = c("g", "o"))
```

Looking at the results it's clear using all the variables is not optimal. 300 variables is the optimum subset from these results, however the accuracy loss by using only 200 variables is minisuclue (<0.001%) and using only 200 variables would speed up compuation so we will use this as our optimum subset of bacterial species for further analysis when aiming to predict city of origin. 
...
Repeat this optimum variable selection for both latitude and longitude. 

```{r}
featureEliminationLat <- rfe(x = metasub_data[, c(43:3799)],y = metasub_data$latitude,
                 sizes = c(100,200,300,400,500),
                 rfeControl = rfe_ctrl,
                 tuneLength = 0
                 )
...
optimumVarsLat <- featureEliminationLat$optVariables

featureEliminationLong <- rfe(x = metasub_data[, c(43:3799)],y = metasub_data$longitude,
                 sizes = c(50,100,200,300,400),
                 rfeControl = rfe_ctrl,
                 tuneLength = 0
                 )

featureEliminationLong

optimumVarsLong <- featureEliminationLong$optVariables
```
...
First split our data into training and test sets. 

```{r}
set.seed(24)
inTrain <- createDataPartition(y = metasub_data$city, p = 0.8, list = FALSE)

training <- metasub_data[inTrain, ]
testing <- metasub_data[-inTrain,]

```

 Using the trainng set and predetermined optimum varibales we can now fit some models, namely XgBoost and random forests. Here we will utilize the caret package for hyper parameter tuning usimng 5 fold cross validation. 
...
#Create trainng folds index, for ensembling later
folds <- createFolds(metasub_data$city, k = 5, returnTrain = T)
...
training <- metasub_data[trainFolds_lon[[8]],]
testing <- metasub_data[-trainFolds_lon[[8]],]

# 5 fold cross validation for hyperparameter tuning xgboost

baseRf <- train(x = metasub_data[,c(optimumVarsCity)],y= metasub_data[,"city"],
   method = "rf",
...
featureElimination$results


baseRf2 <- train(x = cbind(training[,optimumVarsCity],baseRf[["pred"]][order(baseRf$pred$rowIndex),"pred" ]),y= training[,"longitude"],
   method = "rf",
   trControl = trControl,
   tuneLength = 3
...
basexg <- train(x = metasub_data[,c(optimumVarsCity)],y= metasub_data[,"city"],
   method = "xgbTree",
   trControl = trControlClass,
   tuneLength = 2
...
l2_train <- data.frame(metasub_data[,c(optimumVarsCity)] ,baseRf[["pred"]][order(baseRf$pred$rowIndex),levels(metasub_data$city) ] )

basexglat <- train(x = l2_train ,y= metasub_data[,"latitude"],
   method = "xgbTree",
   trControl = trControl,
   tuneGrid = tune_grid
)
...
l3_train <- data.frame(l2_train,basexglat[["pred"]][order(basexglat$pred$rowIndex),"pred" ] )



basexglong <- train(x = l3_train,y= metasub_data[,"longitude"],
   method = "xgbTree",
   trControl = trControl,
   tuneGrid = tune_grid
...
l3_train <- data.frame(training[,c(optimumVarsCity)] ,basexg[["pred"]][order(basexg$pred$rowIndex),levels(training$city) ] )



basesv <- train(x = training[,c(optimumVarsLong)],y = training[,"longitude"],
   method = "svmRadial",
   trControl = trControl,
   tuneLength = 6
)
...
stackData <- data.frame("longitude" = training[,"longitude"],
                        
                        rfPred = baseRf[["pred"]][order(baseRf$pred$rowIndex),"pred" ] ,
                        
                        xgPred = basexg[["pred"]][order(basexg$pred$rowIndex),"pred" ],
                        
                        svPred = basesv[["pred"]][order(basesv$pred$rowIndex),"pred" ],
                        
                        gbmPred = basegb[["pred"]][order(basegb$pred$rowIndex),"pred" ]
                        )

stackData

avg <- ( baseRf2[["pred"]][order(baseRf2$pred$rowIndex),"pred"] +  basexg2[["pred"]][order(basexg2$pred$rowIndex),"pred"] +  basesv[["pred"]][order(basesv$pred$rowIndex),"pred" ] +basesv1[["pred"]][order(basesv1$pred$rowIndex),"pred" ])/4
...
stackData2 <- data.frame("longitude" = training[,"longitude"],
                        
                        rf2Pred = baseRf2[["pred"]][order(baseRf2$pred$rowIndex),"pred" ] ,
                       
                        xg2Pred = basexg2[["pred"]][order(basexg2$pred$rowIndex),"pred" ],
                       
                        sv2Pred = basesv12[["pred"]][order(basesv12$pred$rowIndex),"pred" ]
                        )
...
FinalModel1 <-  train(x = stackData[,-c(1,2)],y= stackData[,"latitude"],
   method = "knn",
...
FinalModel2 <-  train(x = stackData[,-1],y= stackData[,"longitude"],
   method = "knn",
   trControl = stackControl,
   tuneLength = 12)
...
preProcValues <- preProcess(testStack, method = c("center", "scale"))


baseRf[["pred"]][order(baseRf$pred$rowIndex), "pred"]<- predict(baseRf, newdata = testing[,optimumVarsCity])

city_pred <-  predict(basexg, newdata = testing[,optimumVarsCity], type = "prob")

latPred <- predict(basexglat, newdata = data.frame(testing[,optimumVarsCity], city_pred))
longPred <- predict(basexglong, newdata = data.frame(testing[,optimumVarsCity], city_pred, "basexglat...pred....order.basexglat.pred.rowIndex....pred.." = latPred))

gbmPred <- predict(basegb, newdata = testing[,optimumVarsLong])

testStack <- data.frame(rfPred,xgPred,svPred,gbmPred)
testPred <- predict(FinalModel2, newdata = testStack)

RMSE(rfPred, testing[,"longitude"]) 

RMSE(latPred, metasub_data[-trainFolds_lon[[8]],"latitude"])
...
Our model makes use of regressor chains (more info: http://cig.fi.upm.es/articles/2015/Borchani-2015-WDMKD.pdf) in order to preoduce multi output predictions for class (city), latittude and longitude. This techniques involves chaining predictions for a series of outputs as input features for the next level model. 
...
Our function here takes 5 arguments: a training set, a test set, a target variable, a set of variables to use when training the model and a method either "regression or "classification". The function outputs the final layer trained model and the predictions from applying this model to the input test set. 
...
Using our algorithm will will employ strativied 10-fold validation on the metasub data set in order to generate test predictions for city, latitude and longitude.  


```{r}

#generate 10 stratified folds for test predictions.
set.seed(18)
...
For 10-fold test predictions we have an accuracy of 93% for city predictions and RMSE of 8.2 and 23.4 for latitude and longitude respectively. 

Now we have our predicions we can begin to visulize them in order to get a feel for the effectiveness of the algorithm, as in this context RMSE is hard to understand paractially. 
So we will use predicted co-ordinates to a generate the distance of predictions from the true origin. 
...
The mean distance of the origin prediction from the tru origin is 1013km and median 256.7  with 64% of samples within 500km

We can plot the predicted origin of each global sample. Coloured by continent of origin. 
...
#png("World_test.png", width = 13, height = 8, units = 'in', res = 600)
...
for ( i in 1:length(levels(MetasubDataPreds$continent))){
  ...
  find_lats <- MetasubDataPreds[MetasubDataPreds[,"continent"] == this_continent,][which(is.na(GPS_where)),"latPred"]
  find_longs <- MetasubDataPreds[MetasubDataPreds[,"continent"] == this_continent,][which(is.na(GPS_where)),"longPred"]
