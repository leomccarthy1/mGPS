---
title: "Mapping with microbes, machine learning and XBoost predicts location"
output: 
  html_document:
    df_print: paged
---
...
```{r, echo = F}
load("~/Documents/Microbiome mapping/Markdown/GlobalPredObj.RData")
...
Our model makes use of regressor chains (more info: http://cig.fi.upm.es/articles/2015/Borchani-2015-WDMKD.pdf) in order to preoduce multi output predictions for class (city), latittude and longitude. This techniques involves chaining predictions for a series of outputs as input features for the next level model. In this case 3 chained XGboost models are used with hyperparameter tuning carried out by 5-fold cross validation of the trainng set using grid search methods. 

Here this function here takes 4 arguments: a training set, a test set, a target class variable, a set of variables to use when training the model. The function outputs the class,latitude and longitude predictions from applying this model to the input test set. 
...
For 10-fold test predictions we have an accuracy of 93% for city predictions and RMSE of 8.2 and 23.5 for latitude and longitude respectively. 
...
We can plot the predicted origin of each global sample. Coloured by continent of origin. City prediction accuracy is showm by the pie charts. 
...
Further code for plots of the predictions seperated by region can be found in the file "global_plots.R"


#Predictions unseen cities

Here we will test our model on predicting samples from cities that are not seen in the training data. For this stage we ommit the hyper paramter tuning stage in order to save time with computation and only a marginal loss in accuracy. An effective standard set of hyperparameters are assigned instead. 
...
First randomly select 10 cities from the data set. Then create 10 training folds that contain all data other than the data for each of these cities in turn. Test the trained model by generating predictions for samples from the city that was left out, do this for all 10 cities. 
...
```{r, eval = F, echo = F}
palette <-randomColor(count = 10, luminosity = "bright")
png("WorldPredsLO.png", width = 13, height = 8, units = 'in', res = 600)
plot(map, xlim = c(-165,168),ylim = c(-70,60), col = "grey",border = "darkgrey", xlab = "Longitude", ylab = 'Latitude', bg = "lightskyblue1")
for ( i in 1:length(levels(MetasubLeaveOutPreds$city))){
  this_city <- levels(MetasubLeaveOutPreds$city)[i]
  find_lats <- MetasubLeaveOutPreds[MetasubLeaveOutPreds[,"city"] == this_city,][,"latPred"]
  find_longs <-MetasubLeaveOutPreds[MetasubLeaveOutPreds[,"city"] == this_city,][,"longPred"]
...
median(MetasubLeaveOutPreds[MetasubLeaveOutPreds$city == "paris","Distance_from_origin"])
median(MetasubDataPreds[MetasubDataPreds$city == "paris","Distance_from_origin"])
